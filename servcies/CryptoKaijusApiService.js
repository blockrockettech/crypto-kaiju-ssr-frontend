...
class CryptoKaijusApiService {
    ...
    async getHomePageDetails(network = 1) {
        return axios.get(`${API_URL}/network/${network}/homepage`, AXIOS_CONFIG)
            .then((payload) => payload.data);
    }

    async searchByTokenId(network = 1, tokenId) {
        return axios.get(`${API_URL}/network/${network}/search/id/${tokenId}`, AXIOS_CONFIG)
            .then((payload) => payload.data);
    }

    async searchByNfcId(network = 1, nfcId) {
        return axios.get(`${API_URL}/network/${network}/search/nfc/${nfcId}`, AXIOS_CONFIG)
            .then((payload) => payload.data);
    }
}
